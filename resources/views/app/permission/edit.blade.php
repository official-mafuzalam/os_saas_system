    ...
    <x-slot name="main">
        ...
        <div class="w-full sm:pt-5 px-1 sm:px-6 md:px-8">
            ...
            <div class="bg-slate-300 rounded-xl shadow p-4 mb-10 sm:p-7 dark:bg-slate-800">
                ...
                <form action="{{ route('tenant.permission.update', $permission->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <!-- Grid -->
        ...
        <div class="w-full pt-5 px-4 sm:px-6 md:px-8">
            ...
            <div class="bg-slate-300 rounded-xl shadow p-4 mb-10 sm:p-7 dark:bg-slate-800">
                    ...
                    @if ($permission->roles)
                        @foreach ($permission->roles as $permission_role)
                            <form class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-md" method="POST"
                                action="{{ route('tenant.permissions.roles.revoke', [$permission->id, $permission_role->id]) }}"
                ...
                <form method="POST" action="{{ route('tenant.permissions.role', $permission->id) }}">
                    @csrf
                    <div class="grid sm:grid-cols-12 gap-2 sm:gap-6">
